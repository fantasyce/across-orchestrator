...
After `v0.12.1` is published from `origin/main`, install that release tag directly from GitHub:

```bash
python3 -m pip install "git+https://github.com/fantasyce/across-orchestrator.git@v0.12.1"
```

The prepared GitHub release is source-first. There is no attached Python wheel; packaged
hosts can install from the pinned tag after publication. The `v0.12.1` release will publish fresh
