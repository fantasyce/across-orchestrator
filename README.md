...
`v0.12.0` adds explicit, durable Goal revalidation plan/start/complete phases on top of the shared Goal Contract, criterion-bound evidence graph,
...
block publication, signing, and attestation.
...
After `v0.12.2` is published from `origin/main`, install that release tag directly from GitHub:

```bash
python3 -m pip install "git+https://github.com/fantasyce/across-orchestrator.git@v0.12.2"
```

The prepared GitHub release is source-first. There is no attached Python wheel; packaged
hosts can install from the pinned tag after publication. The `v0.12.2` release will publish fresh
