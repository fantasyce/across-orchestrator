...
class CliTests(unittest.TestCase):
    ...
    def test_cli_plugin_manifest_is_json(self):
        ...
        self.assertEqual(
            ...
            "across-goal-revalidation-request/1.1",
        )
    ...
    def test_cli_goal_revalidation_plan_start_and_complete_host_attempt(self):
        payload = {
        ...
        planned = self.run_cli("goal-revalidation", "plan", "--payload-json", json.dumps(payload), "--json")
        self.assertEqual(planned.returncode, 0, planned.stderr)
        plan = json.loads(planned.stdout)
        self.assertEqual(plan["schema_version"], "across-goal-revalidation-plan/1.1")

        start_payload = {
            **payload,
            "execution_mode": "host_validation",
            "idempotency_key": "idem-cli-revalidation-host",
            "plan_hash": plan["plan_hash"],
        }
        started = self.run_cli("goal-revalidation", "start", "--payload-json", json.dumps(start_payload), "--json")
        ...
        self.assertEqual(attempt["state"], "awaiting_host_evidence")
        self.assertEqual(attempt["job_ids"], [])
        ...
        unsigned_receipt = {
            ...
            "task_id": "task-cli",
            "criterion_ids": ["criterion-a"],
            "artifact_digests": {"task-result.md": "b" * 64},
            "input_fingerprint": "a" * 64,
            "validator_id": "aaa-host-validator",
            "verdict": "verified",
        }
        receipt = {**unsigned_receipt, "receipt_hash": payload_hash(unsigned_receipt)}
        completed = self.run_cli(
            "goal-revalidation",
            "complete",
            "--payload-json",
            json.dumps({"attempt_id": attempt["attempt_id"], "receipt": receipt}),
            "--json",
        )
